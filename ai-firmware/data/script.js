...
function handleWsMessage(data) {
  if (data.type === "TELEMETRY") updateTelemetry(data);
    ...
    showToast(`Route nạp: ${data.commands} node`, "success");
  else if (data.type === "OBSTACLE_DETECTED") {
    const obsNode =
      data.obstacleNode !== undefined
        ? data.obstacleNode
        : data.position
          ? data.position.row * 5 + data.position.col
          : -1;
...
function handleDynamicReroute(data) {
  ...
  const obsNode =
    data.obstacleNode !== undefined
      ...
      : data.position
        ...
        : -1;
  const robotNode =
    data.robotNode !== undefined
      ? data.robotNode
      : data.robot_position
        ? data.robot_position.row * 5 + data.robot_position.col
        : -1;
  ...
  if (tarArr.length > 0 && robotNode >= 0) {
    ...
    if (result) {
      ...
      const cppDir =
        data.robotDir !== undefined
          ? data.robotDir
          : JS_TO_CPP_DIR[robotCurrentDir];
      wsSend({
        type: "ROUTE",
        path: result.path,
        initialDir: cppDir,
...
function setMapTool(tool) {
  ...
  edgeSelFirst = null;
  document
    .getElementById("toolStart")
    .classList.toggle("active", tool === "start");
  document
    .getElementById("toolTarget")
    .classList.toggle("active", tool === "target");
  document
    .getElementById("toolObstacle")
    .classList.toggle("active", tool === "obstacle");
  document
    .getElementById("toolEdge")
    .classList.toggle("active", tool === "edge");
  document
    .getElementById("toolWeight")
    .classList.toggle("active", tool === "weight");
  updateEdgeSelUI();
}
...
function handleNodeClick(nid) {
  ...
  if (currentMapTool === "edge") {
    if (edgeSelFirst === null) {
      edgeSelFirst = nid;
      updateEdgeSelUI();
      showToast(`Node ${nid} chọn — click node khác để toggle edge`, "info");
    } else {
      if (edgeSelFirst !== nid) {
        const added = toggleEdge(edgeSelFirst, nid);
        showToast(
          `Edge ${edgeSelFirst}↔${nid} ${added ? "đã thêm" : "đã xóa"}`,
          "success",
        );
      }
      edgeSelFirst = null;
  ...
  if (currentMapTool === "weight") {
    if (edgeSelFirst === null) {
      edgeSelFirst = nid;
      updateEdgeSelUI();
      showToast(`Node ${nid} chọn — click node kề để đặt trọng số`, "info");
    } else {
      if (edgeSelFirst !== nid && graph[edgeSelFirst].includes(nid)) {
        ...
        showToast("Không có edge giữa 2 node này!", "error");
      }
      edgeSelFirst = null;
      updateEdgeSelUI();
    }
...
function updateNodeVisuals() {
  ...
  if (targets.size > 0) {
    ...
    resEl.className =
      "p-4 text-sm text-on-surface-variant text-center font-semibold";
  } else {
...
function findPathToNearest(start, targetSet, algo) {
  ...
  if (algo === "bfs") {
    ...
    for (let t of targetSet)
      pq.push({ path: [start], h: heuristic(start, t), target: t });
    pq.sort((a, b) => a.h - b.h);
    // Use multi-target greedy: h = min heuristic to any remaining target
    pq = [
      {
        path: [start],
        h: Math.min(...[...targetSet].map((t) => heuristic(start, t))),
      },
    ];
    while (pq.length) {
      pq.sort((a, b) => a.h - b.h);
      ...
      for (let nx of shuffle(graph[node] || [])) {
        if (!explored.has(nx) && !obstacles.has(nx)) {
          ...
          pq.push({
            path: [...cur.path, nx],
            h: Math.min(...[...targetSet].map((t) => heuristic(nx, t))),
          });
        }
      }
    }
    ...
    let pq = [
      {
        path: [start],
        g: 0,
        f: Math.min(...[...targetSet].map((t) => heuristic(start, t))),
      },
    ];
    let gS = new Map(),
      explored = new Set(),
      ...
      for (let nx of shuffle(graph[node] || [])) {
        ...
        if (!gS.has(nx) || ng < gS.get(nx)) {
          ...
          pq.push({
            path: [...cur.path, nx],
            g: ng,
            f: ng + Math.min(...[...targetSet].map((t) => heuristic(nx, t))),
          });
        }
      }
    }
...
function buildSegments(sn, tarArr, algo) {
  ...
  for (let tar of ordered) {
    const r = findPathAnimated(currentS, tar, algo);
    if (!r) continue;
    segs.push({
      steps: r.steps,
      path: r.path,
...
function animStart() {
  ...
  const algo = document.getElementById("algoSel").value;
  const { segs, fullPath, ordered, totalSteps, totalExplored, totalCost } =
    buildSegments(sn, tarArr, algo);
  if (!segs.length) {
    showToast("Không tìm được đường!", "error");
    return;
  }
  animStop();
  animClearVis();
...
function animShowFinalPath() {
  ...
  showToast(
    `✅ Hoàn thành! ${anim.resultSteps} bước — ${anim.resultExplored} node duyệt` +
      (anim.segCount > 1 ? ` (${anim.segCount} đoạn)` : ""),
    "success",
  );
}
...
function calculateOverallPath() {
  ...
  if (!targets.size) {
    resEl.innerText = "Chọn điểm giao hàng trên bản đồ";
    resEl.className = "p-4 text-sm text-on-surface-variant text-center";
  ...
  if (deliveryMode === "express" && targets.size > 1) {
    const tarArr = Array.from(targets),
      expressTarget = tarArr[tarArr.length - 1];
    const er = findPath(sn, expressTarget);
    if (er) {
      ordTar.push(expressTarget);
      segs.push(er.path);
      fullPath = er.path;
      totalSteps += er.path.length - 1;
      totalVis += er.visitedCount;
      curS = expressTarget;
    ...
    while (rem.size) {
      let best = null,
        ...
        sl = Infinity;
      for (let t of rem) {
        let r = findPath(curS, t);
        if (r && r.path.length < sl) {
          sl = r.path.length;
          best = r;
          bestT = t;
        }
      ...
      totalSteps += best.path.length - 1;
      totalVis += best.visitedCount;
      curS = bestT;
      rem.delete(bestT);
    }
  } else {
    while (rem.size) {
      ...
      segs.push(best.path);
      fullPath =
        fullPath.length > 1 ? fullPath.concat(best.path.slice(1)) : best.path;
      totalSteps += best.path.length - 1;
      totalVis += best.visitedCount;
      curS = bestT;
      rem.delete(bestT);
  ...
  if (ordTar.length) {
    ...
    document.getElementById("autoOrderNum").textContent =
      "#01/" + (ordTar.length < 10 ? "0" + ordTar.length : ordTar.length);

    let legHTML =
      '<div class="w-full text-center font-bold text-xs text-on-surface-variant mb-1">THỨ TỰ GIAO HÀNG';
    if (deliveryMode === "express")
      legHTML += ' <span class="text-red-500">(CHẾ ĐỘ HỎA TỐC)</span>';
    legHTML += "</div>";
    ordTar.forEach((node, idx) => {
      ...
      poly.setAttribute("points", pts);
      poly.setAttribute("stroke", col);
      poly.setAttribute("style", `filter:drop-shadow(0 0 4px ${col}40)`);
      ...
      if (idx < ordTar.length - 1)
        legHTML += '<span class="text-outline-variant">➔</span>';
    });
    ...
    resEl.innerText = "LỖI: Không tìm được đường! Vật cản chặn toàn bộ.";
    resEl.className = "p-4 text-sm text-center err-msg";
    legendBox.classList.add("hidden");
    finalCalculatedPath = [];
...
document.getElementById("deliverBtn").addEventListener("click", async () => {
  if (finalCalculatedPath.length < 2)
    return showToast("Lộ trình không hợp lệ!", "error");
  ...
  if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    try {
      await fetch(`/deliver?dir=1&path=${finalCalculatedPath.join(",")}`);
    } catch (e) {}
  }
  setTimeout(() => {
    ...
    setTimeout(() => {
      btn.innerHTML = "▶ NẠP LỘ TRÌNH & BẮT ĐẦU";
    }, 2000);
  }, 1000);
});
...
document.getElementById("compareAllBtn").addEventListener("click", () => {
  ...
  const tarArr = Array.from(targets);
  if (!tarArr.length) {
    showToast("Chọn điểm giao!", "error");
    return;
  }
  ...
  for (let algo of algos) {
    ...
    let tv = 0,
      ...
      cs = sn;
    for (let t of tarArr) {
      const r = findPath(cs, t, algo);
      if (r) {
        tv += r.visitedCount;
        tc += r.cost || r.path.length - 1;
        pl += r.path.length - 1;
        cs = t;
      } else {
        ok = false;
    ...
    results.push({
      ...
      pathLen: pl,
      cost: tc,
      time: parseFloat((performance.now() - t0).toFixed(3)),
      success: ok,
    });
  }
  results.sort((a, b) => {
  ...
  algoChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Số bước",
          data: results.map((r) => (r.success ? r.pathLen : 0)),
        ...
        {
          label: "Node duyệt",
          data: results.map((r) => (r.success ? r.visited : 0)),
        ...
        {
          label: "ms",
          data: results.map((r) => (r.success ? r.time : 0)),
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          grid: { color: "#e1e3e4" },
          ticks: { color: "#414753", font: { size: 11 } },
        },
        x: {
          grid: { display: false },
          ticks: { color: "#414753", font: { size: 12, weight: "bold" } },
        },
      },
      plugins: {
        legend: {
        ...
        tooltip: { backgroundColor: "#191c1d", cornerRadius: 10, padding: 12 },
      },
    },
  });
  ...
  for (let r of results) {
    ...
    const badge = r.success
      ? '<span class="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-[10px] font-bold">✓</span>'
      : '<span class="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-[10px] font-bold">✗</span>';
