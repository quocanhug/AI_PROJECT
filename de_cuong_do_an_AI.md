...
| 9 | **Dynamic Obstacle Re-routing** | Khi gặp vật cản bất ngờ: ESP32 gửi tín hiệu về Web → Web hiển thị vật cản + tính lại đường đi mới → Gửi route mới cho ESP32, thời gian phản hồi ≤ 3 giây. |
...
│              │     │     real-time    │     │    về web             │
│              │     │                  │     │                      │
│              │     │ === DYNAMIC OBSTACLE RE-ROUTING LOOP ===      │
│              │     │                  │     │                      │
│              │     │ 11. Nhận tín hiệu│◀────│ 11a. Gặp vật cản    │
│              │     │     OBSTACLE +   │ WS  │      bất ngờ:        │
│              │     │     vị trí (r,c) │     │   → Dừng robot       │
│              │     │                  │     │   → Xác định vị trí  │
│              │     │ 12. Cập nhật bản │     │     trên grid (r,c)  │
│              │     │     đồ: đánh dấu │     │   → Gửi WebSocket   │
│              │     │     ô (r,c) là   │     │                      │
│              │     │     vật cản MỚI  │     │                      │
│              │     │                  │     │                      │
│              │     │ 13. Tự động chạy │     │                      │
...
│              │     │     tại → đích   │     │                      │
│              │     │                  │     │                      │
│              │     │ 14. Gửi đường đi│────▶│ 15. Nhận route mới  │
│              │     │     mới cho ESP32│ WS  │     → Thay thế queue │
│              │     │                  │     │     → Tiếp tục chạy  │
└──────────────┘     └──────────────────┘     └──────────────────────┘
...
| **Vật cản bất ngờ** | **HC-SR04 + Encoder** | **Khi phát hiện** |

#### 7.1.6. Dynamic Obstacle Re-routing — Tái định tuyến khi gặp vật cản bất ngờ

Đây là chức năng **nổi bật** của hệ thống, cho phép robot và web **phối hợp hai chiều (bidirectional)** để xử lý vật cản không có trên bản đồ ban đầu.

**Luồng xử lý chi tiết:**

```
  ESP32 (Robot)                          WEB BROWSER
  ─────────────                          ───────────
  1. Đang chạy theo route              
     (bước thứ k / N)                  
         │                              
  2. HC-SR04 phát hiện vật cản          
     < 15 cm → DỪNG ngay              
         │                              
  3. Xác định vị trí hiện tại          
     trên grid (row, col) dựa vào:    
     • Intersection counter             
     • Bước hiện tại trong route       
         │                              
  4. Gửi WebSocket ──────────────────▶  5. Nhận message OBSTACLE
     {                                     │
       "type": "OBSTACLE_DETECTED",       6. Hiệu ứng cảnh báo:
       "position": {"row":2,"col":3},        • Flash màu đỏ trên ô (r,c)
       "current_step": 3,                    • Animation vật cản ⚠️
       "robot_position": {"row":2,"col":2},  • Âm thanh cảnh báo
       "distance_cm": 12.5                   • Toast notification
     }                                     │
                                          7. Cập nhật grid map:
                                             grid[r][c] = WALL (vật cản mới)
                                             Thay đổi màu ô thành ⬛
                                             │
                                          8. Chạy lại thuật toán AI:
                                             • Start = vị trí robot hiện tại
                                             • End = đích ban đầu
                                             • Grid = bản đồ đã cập nhật
                                             • Animation tìm đường mới
                                             │
                                          9. Convert path → commands mới
  11. Nhận route mới  ◀──────────────── 10. Gửi WebSocket NEW_ROUTE
      │                                    {
  12. Thay thế route queue                   "type": "ROUTE",
      bằng commands mới                      "commands": ["L","F","R","F","F"],
      │                                      "rerouted": true
  13. Tiếp tục chạy                        }
      (PID line following)
```

**Hiển thị trên Web khi gặp vật cản bất ngờ:**

| Thành phần UI | Mô tả |
|---|---|
| **Popup cảnh báo** | "⚠️ Vật cản bất ngờ tại ô (2,3)! Đang tính lại đường đi..." |
| **Bản đồ cập nhật** | Ô mới được đánh dấu vật cản (⬛) với viền đỏ nhấp nháy để phân biệt với vật cản đặt sẵn. |
| **Animation tìm đường** | Chạy lại animation thuật toán AI từ vị trí robot hiện tại, người dùng thấy quá trình tìm đường mới. |
| **Đường đi mới** | Hiển thị đường đi mới (🟩) thay thế đường cũ; đường cũ đã đi (✅) vẫn được giữ lại. |
| **Log sự kiện** | Ghi lại lịch sử: thời gian, vị trí vật cản, thuật toán dùng, đường đi cũ vs mới. |

**Xử lý trường hợp đặc biệt:**

| Trường hợp | Cách xử lý |
|---|---|
| Không tìm được đường đi mới (bị bao vây) | Web thông báo "Không có đường đi!" → Robot dừng hẳn → Người dùng phải bỏ bớt vật cản trên bản đồ hoặc dọn vật cản thực tế. |
| Nhiều vật cản liên tiếp | Mỗi lần gặp vật cản mới → lặp lại quy trình re-routing từ vị trí hiện tại. Bản đồ tích lũy tất cả vật cản phát hiện được. |
| Mất kết nối WebSocket | ESP32 tự chuyển sang chế độ an toàn (dừng), cố gắng kết nối lại. Web hiển thị "Mất kết nối". |
...
│    • Tính vị trí vật cản trên grid (row, col)      │
│    • Gửi OBSTACLE_DETECTED về Web (kèm tọa độ)    │
│    │                                                │
│    ├─(nhận NEW_ROUTE)────▶ STATE_REROUTING          │
│    ├─(timeout 10s)───────▶ Fallback: dừng/vòng tránh│
│    └─(vật cản dọn + Resume)─▶ STATE_FOLLOWING_LINE │
│                                                     │
│  STATE_REROUTING: ★ MỚI                            │
│    • Nhận route mới từ Web                         │
│    • Xóa route queue cũ                            │
│    • Nạp commands mới vào queue                    │
│    • Gửi ACK về Web                                │
│    └──────────────────────▶ STATE_FOLLOWING_LINE    │
│                                                     │
│  STATE_DONE:                                        │
...
#### 7.2.4. Tránh vật cản & Gửi tín hiệu về Web (Dynamic Re-routing)

Nếu HC-SR04 phát hiện vật cản < 15 cm khi đang dò line:

1. **Dừng ngay lập tức.**
2. Servo SG90 quét trái (150°) và phải (30°) → đo khoảng cách 2 phía.
3. **Xác định vị trí vật cản trên grid**: Dựa vào bước hiện tại (`current_step`) trong route queue và hướng đi, ESP32 tính toán tọa độ ô grid (row, col) phía trước robot.
4. **Gửi tín hiệu OBSTACLE_DETECTED về Web** qua WebSocket:

```json
{
  "type": "OBSTACLE_DETECTED",
  "position": {"row": 2, "col": 3},
  "robot_position": {"row": 2, "col": 2},
  "current_step": 3,
  "distance_cm": 12.5,
  "scan": {
    "left_cm": 45.0,
...
5. **Chờ đường đi mới từ Web**: ESP32 dừng và đợi Web xử lý:
   - Web cập nhật bản đồ → đánh dấu ô vật cản
   - Web chạy lại thuật toán AI từ vị trí robot → đích
   - Web gửi route mới cho ESP32

6. **Nhận route mới và tiếp tục**:

```json
// Web gửi lại route mới
{
  "type": "ROUTE",
  "commands": ["L", "F", "R", "F", "F"],
  "total_steps": 5,
...
```
   ...
   ESP32 thay thế route queue hiện tại bằng commands mới → chuyển về `STATE_FOLLOWING_LINE`.

7. **Fallback** — Nếu Web không phản hồi trong 10 giây:
...
| **Tuần 3** | **Tích hợp hệ thống** | • Cấu hình ESP32 Web Server + WebSocket. <br>• Lập trình Route Sender (web) + Route Interpreter (ESP32). <br>• Triển khai State Machine: giao lộ detection, xử lý lệnh F/L/R. <br>• Lập trình tránh vật cản (Servo quét + HC-SR04). <br>• **Dynamic Obstacle Re-routing**: ESP32 gửi vị trí vật cản → Web hiển thị + tính lại đường + gửi route mới. <br>• Xây dựng Telemetry Dashboard (real-time sensor data). <br>• Test tích hợp: web gửi route → robot chạy đúng trên track. | Hệ thống end-to-end hoạt động: Web AI ↔ ESP32 ↔ Robot (hai chiều). |
...
| Sản phẩm | Mô tả |
|---|---|
| **Giao diện Web** | Trang web hoạt động trên trình duyệt, cho phép vẽ bản đồ, chạy mô phỏng 5 thuật toán AI với animation, so sánh hiệu năng, gửi lộ trình cho robot, **nhận thông báo vật cản bất ngờ từ ESP32 + tự động tái định tuyến**, và giám sát real-time. |
| **Firmware ESP32** | Chương trình C++ hoàn chỉnh: nhận lệnh WebSocket, PID line following, intersection handling, obstacle avoidance, **dynamic re-routing** (gửi vị trí vật cản về web + nhận route mới), encoder odometry, telemetry reporting. |
...
| **Dynamic Re-routing** | **Khi gặp vật cản bất ngờ: ESP32 gửi tín hiệu → Web hiển thị vật cản mới + tính lại đường → gửi route mới cho ESP32 trong ≤ 3 giây. Thành công ≥ 90% các lần test.** |
